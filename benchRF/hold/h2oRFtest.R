...
df <- h2o.importFile(path = normalizePath("~/benchRF/res/mnist.csv"))
...
rf1 <- h2o.randomForest(         ## h2o.randomForest function
  ...
  validation_frame = train,      ## the H2O frame for validation (not required)
  x=2:785,                        ## the predictor columns, by column index
  y=1,                          ## the target index (what we are predicting)
  model_id = "rf_covType_v1",    ## name the model in H2O
                                 ##   not required, but helps use Flow
  ntrees = 10,                  ## use a maximum of 200 trees to create the
  ...
  score_each_iteration = F,      ## Predict against training and validation for
