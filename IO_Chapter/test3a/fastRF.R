...
time <- 0
sampSize <- 5000000
binSize <- c(.00001,.0001,.001,.01,.1,1)
	...
	for (i in 1:nTimes){
		for (bs in binSize){
			...
			for (algName in c("rfBase","rerf")){
				...
				forest <- fpRerF(X =X, Y = Y, forestType=algName,minParent=1,numTreesInForest=num_trees,numCores=p,nodeSizeToBin=bs*samples, nodeSizeBin=bs*samples)
				ptm_hold <- (proc.time() - ptm)[3]

				predictions <- fpPredict(forest, Xt)
				error <- sum(predictions == Yt)/length(Yt)

				resultData <- rbind(resultData, c("airline",algName,samples,ptm_hold,i,error,bs*100)) 
	...
	for (i in 1:nTimes){
		for (bs in binSize){
			...
			for (algName in c("rfBase","rerf")){
				...
				resultData <- rbind(resultData, c("Higgs 10M",algName,samples,ptm_hold,i,error,bs*100)) 
