	...
	for (p in ML){
		for(j in binSizePercent){
			...
			for (i in 1:nTimes){
				...
				resultData <- rbind(resultData, c("MNIST", "fastRF(Bin)",j, ptm_hold,error)) 
				rm(forest)
			}
		}
	}
	...
	for (p in ML){
		for(j in binSizePercent){
			...
			for (i in 1:nTimes){
				...
				ptm_hold <- (proc.time() - ptm)[3]
				resultData <- rbind(resultData, c("higgs", "fastRF(Bin)",j, ptm_hold,error)) 
				rm(forest)
			}
		}
	}
	...
	for (p in ML){
		for(j in binSizePercent){
			...
			for (i in 1:nTimes){
				...
				error <- sum(predictions==Yt)/length(Yt)
				ptm_hold <- (proc.time() - ptm)[3]
				resultData <- rbind(resultData, c("p53", "fastRF(Bin)",j, ptm_hold,error))  
				rm(forest)
			}
		}
	}
