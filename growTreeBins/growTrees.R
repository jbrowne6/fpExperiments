...
for (alg in c("rerf", "rfBase")){
	for (num_trees in c(1,2,4,8,16,32,64,128)){
			for (j in 1:nTimes){
				...
				forest <- fpRerF(X =X, Y = Y, forestType=alg,minParent=1,numTreesInForest=num_trees,numCores=1)
				ptm_hold <- (proc.time() - ptm)[3]

				predictions <- fpPredict(forest, Xt)
				error <- sum(predictions==Yt)/length(Yt)

				resultData <- rbind(resultData, c("MNIST", alg, ptm_hold,error, 0,0,num_trees)) 
...
for (alg in c("rerf", "rfBase")){
	for (num_trees in c(1,2,4,8,16,32,64,128)){
		for (binS in c(300,600,900,1200)){
			for (j in 1:nTimes){
				gc()
				ptm <- proc.time()
				#		forest <- RerF(X,Y, trees=num_trees, bagging=.3, min.parent=1, max.depth=0, store.oob=TRUE, stratify=TRUE, num.cores=p, seed=sample(1:100000,1))
				forest <- fpRerF(X =X, Y = Y, forestType=alg,minParent=1,numTreesInForest=num_trees,numCores=1,nodeSizeToBin=binS, nodeSizeBin=binS)
				ptm_hold <- (proc.time() - ptm)[3]

				predictions <- fpPredict(forest, Xt)
				error <- sum(predictions==Yt)/length(Yt)

				resultData <- rbind(resultData, c("MNIST", alg, ptm_hold,error, binS,binS,num_trees)) 
